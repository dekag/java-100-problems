...
public class MutableJson {
	...
	/*
	 * public static void main(String[] args) { MutableJson json = new MutableJson( new HashMap<String, Object>() {{
	 * put("store", new HashMap<String, Object>() {{ put("name", "Some Store"); put("books", Arrays.asList( new
	 * HashMap<String, Object>() {{ put("isbn", "111"); }}, new HashMap<String, Object>() {{ put("isbn", "222"); }} )); }});
	 * }} );
	 * 
	 * System.out.println("Before:\t" + json.map());
	 * 
	 * json.update("$.store.name", "Book Store"); json.update("$.store.books[0].isbn", "444");
